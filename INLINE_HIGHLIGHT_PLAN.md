...
### 3.1 발화(재파싱·재틴트) 트리거 — 불변식 #3 우선

목표 3("미완성 조합 문자에 서식 금지")이 UX 편의보다 **우선**한다. 500ms 정지는 조합 확정이
아니므로 조합 중에는 칠하지 않는다. 트리거를 **조합 경로 / 비조합 경로**로 분리한다:

1. **`_isComposing`이면 재틴트하지 않는다.** (조합 중 음절을 절대 건드리지 않음)
2. **재틴트 트리거 = `공백(비조합)` | `idle(비조합)` | `TextCompositionEnded`.** 조합 중 idle이
   떠도 **취소**하고 `TextCompositionEnded` 후 다시 실행한다.
   - 조합 입력은 CompositionEnded가, 조합을 안 거치는 편집(붙여넣기·삭제·화살표·숫자/영문)은
     공백·idle이 덮는다.
   - 한글은 **음절마다 CompositionEnded**가 나므로 앞 토큰은 그 음절 확정 시 켜지고, **맨 끝
     미확정 음절만** 확정(공백/Enter/포커스) 때까지 대기한다 — 그게 곧 건드리면 안 되는 음절.
     즉 "띄어쓰기 해야 켜진다"가 아니라 "확정되는 순간 켜진다".
3. **조합 중 Enter = IME 확정만, task commit 아님.** `QuickAdd_KeyDown`에서 `_isComposing`이면
   return(대개 플랫폼이 이미 그 Enter를 IME가 소비). 확정 후 다음 Enter가 commit.
4. **재파싱 1회/버전.** 실제 텍스트 변경 때만 `_docVersion`을 올리고, 재틴트가
   `_lastParsedVersion == _docVersion`이면 skip한다 — 공백·CompositionEnded·idle이 같은 최종
   텍스트에 몰려도 1회만 재파싱(서식 적용이 비동기로 `TextChanged`를 재발생시키는 루프도 차단).

> **표시 ≠ 진실:** 맨 끝 음절이 잠깐 늦게 칠해져도, 커밋은 단계 5.2대로 **확정 후 재파싱**하므로
> 저장된 task는 항상 정확하다. 그래서 #3을 우선해도 기능 손해가 없다.
