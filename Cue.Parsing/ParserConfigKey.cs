...
/// the bare-hour afternoon toggle plus the user's custom date meanings. A caller (the app's preference-backed
/// parser) reuses one parser while this key is unchanged and rebuilds it when the key changes.
/// </summary>
/// <remarks>
...
public static class ParserConfigKey
{
    ...
    public static string Build(bool autoAfternoonForBareOneToSix, IEnumerable<(string Name, int DayOfMonth)> customDateMeanings)
    {
        var sb = new StringBuilder();
        sb.Append(autoAfternoonForBareOneToSix ? '1' : '0').Append('|');
