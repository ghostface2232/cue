...
* 현재 보고 있는 목록, 그룹, 태그에 새 할 일을 즉시 추가할 수 있는 빠른 한 줄 입력창. 입력하는 동안 파서가 인식한 날짜·시간·반복·"언젠가" 표현을 그 자리에서 강조(accent) 색으로 칠해 보여주며, 한글 IME 조합 중에는 다시 칠하거나 Enter로 확정하지 않습니다(조합이 끝난 다음에만). 강조된 토큰을 누르면 작은 메뉴가 떠 해석 결과를 확인하고(예: `내일 → 6월 30일 (화)`), 다른 날짜·시간·반복으로 빠르게 바꾸거나 `일반 텍스트로 입력`을 골라 그 단어를 인식 대상에서 빼고 제목에 그대로 남길 수 있습니다.
...
ViewModels         MVVM 뷰 모델 레이어 (빠른 입력 토큰화/미리보기, 되돌리기 추적, 빠른 교정 대안 등 포함)
Pages/             XAML 페이지 및 화면 코드비하인드
Controls/          재사용 커스텀 컨트롤 (인라인 하이라이트·교정·IME 처리를 담은 빠른 입력창 OmniInputBox 등)
