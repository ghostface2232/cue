...
### 단계 5 후속 — 코드 리뷰 대응(P1 3건) ✅
- **중복 커밋 직렬화:** 컨트롤 Enter 경로는 `AddCommand`(AsyncRelayCommand 자체 재진입 가드)를 우회해 `SubmitQuickAddAsync`를 직접 호출 → 저장 await 중 두 번째 Enter가 **동일 라인을 재파싱해 중복 작업 생성** 가능. VM에 `_isSubmitting` 게이트 추가(단일 UI 스레드라 평이한 bool로 충분). 커밋 코어를 `SaveQuickAddAsync`로 분리해 try/finally로 감쌈.
- **저장 중 입력 보존:** 저장 후 `QuickAddText`를 무조건 비우면 저장 사이 입력이 유실 → 제출 라인과 현재 박스 텍스트가 **같을 때만** 비우도록 조건화. 미사용이던 `QuickAddSubmission.DocumentVersion`은 제거(스테일 처리를 텍스트 동일성으로 일원화; 컨트롤 `DocumentVersion` 공개 속성도 제거, 틴트 dedup용 `_documentVersion` 필드는 유지).
- **낡은 토큰 범위 치환 방지:** 비공백 편집은 재틴트를 500ms 지연 → 그 사이 `_tokens`가 옛 offset 유지 → 탭 시 잘못된 글자 치환/되돌리기. 탭 핸들러에서 hit-test 전 **대기 중 재틴트를 동기 flush**(`_idle.Stop()` + `ReTint()`, 이미 최신이면 no-op)해 토큰을 라이브 텍스트와 정합. `ReplaceToken`은 쓰기 직전 해당 span의 현재 텍스트가 `token.Text`와 일치하는지 재확인(불일치 시 중단)하는 가드 추가.
- **±1시간 자정 횡단 버그:** 시(時)만 mod-24 순환하고 날짜 토큰은 안 바꿔, `오후 11시`의 "1시간 늦게"가 같은 날 00시(=23시간 이르게)로 어긋남. ±1시간은 **같은 날에 머무를 때만** 제시(00시 일찍/23시 늦게 생략). ±12h 오전/오후 토글은 자정을 넘지 않음(불변).
- **테스트 가능성 추출:** 컨트롤의 순수 대안 생성 로직(`AlternativesFor`/요일·시각·반복)을 `Cue.Tests`가 참조하는 `Cue.ViewModels`로 이전(`QuickAddAlternatives`, `SuppressionTracker` 선례) — 기존엔 손으로 베낀 문자열만 파서로 검증돼 자정 같은 **의미 버그를 못 잡던 갭**. 신규 `QuickAddAlternativesTests`는 생성된 치환을 실제 파서로 해석해 **최종 시각 instant**(±1h·자정 경계·플립)를 검증. VM 동시성/조건부 비우기는 지연 가능한 `GatedSaveStore` 데코레이터로 결정적 레이스 검증. **421 테스트 통과**(빌드 0/0).

### 단계 5 후속 — 코드 리뷰 2차(P1 2건·P2 1건) ✅
- **재틴트 Undo 그룹화(P1):** 재틴트는 전체 default 리셋·토큰 액센트·caret 서식 초기화 + 선택 영역 2회 이동을 수행하는데, TOM상 **삽입 지점(caret) 변경은 현재 undo 그룹을 종료**시키므로(MS 문서 확인) 한 번의 재틴트가 undo 스택을 *눈에 안 보이는* 여러 항목으로 쪼갤 수 있음(Step 0의 "Ctrl+Z 먹통" 위험). 포맷 배치를 `BeginUndoGroup()`/`EndUndoGroup()`로 **하나의 undo 단위**로 묶음(`EndUndoGroup`은 finally로 보장 — 누락 시 이후 타이핑 전체가 한 단위로 뭉침). *트레이드오프 존중*: RichEditBox+CharacterFormat 구조는 그대로, 배치만 원자화. 재틴트를 undo 스택에서 완전히 빼는 건 WinUI 투영에 tomSuspend가 없어 불가(`SetUndoLimit(0)`은 타이핑 이력까지 소거 → 부적절). **체감 지연 여부의 최종 확인은 수동 Ctrl+Z 검증**(아래 체크리스트).
- **억제 span 모호성 처리(P1):** `SuppressionTracker`가 최장 공통 prefix/suffix로만 편집 위치를 추론 → 경계에 반복 문자열이 있으면 위치가 **본질적으로 모호**(`내일 내일`→`내일 내일 내일`은 앞/뒤 삽입 구분 불가, 서로 다른 `내일`을 억제). prefix-우선·suffix-우선 **양방향으로 편집 영역을 구해 합집합을 "불확실 구간"**으로 보고, 그 구간과 겹치는 span은 **이동 대신 폐기**(엉뚱한 반복 토큰 억제 방지). 불확실 구간 밖의 span은 종전대로 결정적으로 이동/유지 → 기존 10개 테스트 불변, 모호/비모호 케이스 2건 추가.
- **파서 캐시 키 충돌(P2):** `PreferenceDateParser`가 사용자 정의 이름을 escaping 없이 `name=day;`로 연결 → `{"a"→1,"b"→2}`와 `{"a=1;b"→2}`가 같은 키(`a=1;b=2;`)를 생성, 다른 설정에 같은 파서가 서빙됨. 키 생성을 `Cue.Parsing.ParserConfigKey`(순수·테스트 가능)로 추출하고 이름을 **길이 prefix**(`{len}:{name}={day};`)로 직렬화 → 구분자 포함 이름도 충돌 불가. `ParserConfigKeyTests` 4건(충돌 회피·동일 입력·플래그·순서/일자 변화). **427 테스트 통과**(빌드 0/0).
...
- [ ] **긴 한글 복합 입력 후 Ctrl+Z** — 한 번에 직전 타이핑이 되돌려지는지(재틴트가 별도 undo 항목으로 끼어들어 "먹통"처럼 보이지 않는지). 리뷰 2차 undo 그룹화의 체감 검증.
- [ ] **알려진 갭:** 토큰은 아직 단색뿐(밑줄/굵기 등 색 외 신호 미적용, 계획 목표 2 — 후속).

---

## 진행 상태
- **완료(코드+CI):** 단계 0~4, 단계 5(5.1 plumbing/커밋, 5.2 팝오버 + 내용 재정리·값 기반 대안, 서식 UI 차단, 리뷰 1·2차 P1 5건·P2 1건 대응). 빌드 그린, 427 테스트 통과.
