...
Domain types are pure C# with zero knowledge of persistence: `TaskItem`, `Project`, `Label`. The hierarchy is one level deep — a `Project` contains `TaskItem`s — and `Label` is cross-cutting. A task's container is a single nullable `ProjectId`; null means unclassified (the Cue home / Inbox). A `Project` is a pure group: it carries no date and no completion state, and an unused group is simply soft-deleted. An Area level above Project is intentionally deferred for the foundation phase — it can be added later as an additive record type plus an optional `Project.AreaId`, with no rewrite.
...
- Queries split on two axes. Classification: by project, by label, and the project-less Cue home (Inbox). Active list queries return non-deleted rows and intentionally keep completed rows visible/dimmed; open-only counts are used for navigation badges. Time, all computed from the single `When` date: Today (a When day today-or-earlier — overdue rolls forward), Upcoming (a future When day), Anytime / "언젠가" (Unscheduled), Logbook (completed). The Timeline view is also When-driven: a task with an OnDate When appears as a single point on that date.
...
- Container deletion policy (resolved): deleting a Project preserves its live and completed tasks by clearing `ProjectId`, moving them to the Cue home (Inbox). This intentionally favors the least destructive behavior over cascading task deletion; the crash-safe deletion saga is retained. Deleting a Label only removes that Label id from tasks and soft-deletes the Label record.
