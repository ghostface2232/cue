...
- Implementation: `SqliteTaskIndex` (raw SQL, the database lives at `{root}/index.db`) holds the query surface, and `IndexedTaskStore` composes a `FileTaskStore` with it so there is exactly one write path — Save/Delete write the file first, then reflect that one record into the index. `OpenAsync` wires both and rebuilds at startup. By-id and GetAll reads come from the files; all *list* reads come from the index (`ITaskIndex`), which returns the lightweight `TaskListItem` projection and never scans the folder.
- Queries split on two axes. Classification: by project, by section, by label, and the project-less Inbox. Time: Today (OnDate whose day is today or earlier and not completed — overdue rolls forward), This Evening (the evening-flagged subset of Today), Upcoming (a future When day or a future Deadline), Anytime (Unscheduled and open), Someday, Logbook (completed). The two axes overlap intentionally (e.g. a deadline-only task is both Anytime and Upcoming).
