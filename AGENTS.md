...
- Design: `KoreanDateParser` is an ordered pipeline of `IQuickAddRule`s (the boost seam, modeled on chrono's parser list). The built-in rules do the Korean recognition — Someday, Recurrence, RelativeHour, Deadline (까지/마감/안에), WhenDate (relative/weekday/absolute + time/evening), TimeOfDay — each consuming the span it recognizes so the leftover becomes the title. Custom rules passed to the constructor run *first*, so they override/extend without touching the defaults. The Recognizers library is still wired in (per spec, with `Culture.Korean`) as a final fallback stage; it contributes nothing for Korean now but lights up automatically if the library gains support.
...
- Known boost-layer gaps (library can't help; add rules as needed): bare day-parts like 점심때/새벽 carry no precise time; some Korean-specific forms (아침저녁 twice-daily, 말일/월급날 semantic dates) aren't mapped yet. PARSING.md is the regression corpus — add a row, add a rule, re-test.
## UI
...
- Current state — minimal data-loop UI (craft deferred; this stage proves the loop end to end): a `NavigationView` with two index-backed views, **Cue** (Inbox — project-less open tasks) and **Today**, both the same `TaskListPage` in different modes via the nav parameter. Each page has a one-line quick-add at the top (Enter → parse → build `TaskItem` → `ITaskStore.Save` → reload from the index) and a `ListView` of `TaskRowViewModel`s (title, a scheduled/deadline caption, and a completion checkbox that sets/clears `CompletedAt` through the store; completing drops the row from the active view on reload). Services are wired through `Microsoft.Extensions.DependencyInjection` in `App`: `IDateParser`, `TimeProvider`, `TimeZoneInfo`, and the one store instance exposed as both `ITaskStore` and `ITaskIndex`; the store is opened (and the index rebuilt) before the window is created, and `TaskListViewModel` is resolved per navigation with these injected. Visuals are stock Fluent/Mica — colors, spacing, motion intentionally not set yet.
