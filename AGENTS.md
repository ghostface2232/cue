...
- **Checklist** is an embedded, ordered `List<ChecklistItem>` on a `TaskItem`. A `ChecklistItem` is a lightweight value (title + `IsChecked` flag + optional memo) — *not* a record: it has no own file, no dates/tags/priority/recurrence, cannot nest, and is persisted as part of the parent task's single JSON. Order is the list position (the parent file is rewritten atomically), so a checklist item carries no `SortOrder`. The list view mirrors each task's checklist as nested rows via a derived JSON column in the index; the detail panel reads/edits it straight off the loaded `TaskItem`.
