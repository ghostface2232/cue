...
- Queries split on two axes. Classification: by project, by label, and the project-less Cue home (Inbox). Time, all computed from the single `When` date: Today (an open task whose When day is today-or-earlier — overdue rolls forward), Upcoming (a future When day), Anytime / "언젠가" (Unscheduled and open), Logbook (completed). The Timeline view is also When-driven: a task with an OnDate When appears as a single point on that date.
