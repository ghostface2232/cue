...
- **Completing a task plays a brief in-row acknowledgement; what happens next depends on whether the task is terminal or repeating.** On tick the circular check pops and the title/meta dim to the completed tone, then the row swaps its body for an acknowledgement bar.
  - **Terminal completion** (a one-off, a repeating series whose rule has run out, or a task completed after its repeat was stopped via **반복 종료**) shows a filled check + "할 일을 완료했습니다" + an **실행 취소** (undo) button. After ~2s (extendable by hovering/pressing the row) the row **folds away** (scale-Y collapse + fade) and the list reloads, dropping it into its 완료한 일 section / Logbook.
...
- **Checking a repeating task records the current cycle, it does not complete the task.** The checkbox means "did this cycle" — it records the cycle (with its checklist frozen) and rolls the same row to the next occurrence; the series stays open. "완료" is reserved for the current cycle, never the series: ending a series (**반복 종료**) only *stops the repeat* (the task becomes a plain open one), and a recurring task reaches 완료한 일 only once its rule naturally runs out or the user completes it after stopping the repeat.
...
### 반복 종료 / 삭제 (Stop recurrence & delete)
- The detail panel's bottom actions are **explicit**, not a hidden long-press. A recurring task shows a **반복 종료** button (neutral 1px-stroke, a flag glyph) above the destructive **할 일 삭제**. "완료" is intentionally avoided for the series — that word means the current cycle — so the action reads as 종료. It **stops the recurrence**: the task becomes a plain, still-open one at its current cycle (it is *not* completed, does *not* go to 완료한 일), and the cycle history is kept. A plain confirm popover ("반복을 종료할까요? 현재 할 일은 일반 할 일로 남습니다.") guards it (not the red delete tone). 반복 종료 also appears in the row's right-click menu for a recurring task, and is equivalent to clearing the 반복 selection ("반복 안 함") in the 일시 card. Deleting is the separate destructive soft-delete.
