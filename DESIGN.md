...
- Sidebar rows start from the stock 4px hover/selection pill gutter. The right gutter has a 1px optical compensation against the content separator. The `CueNav*` values in `DesignTokens.xaml` are corrections for the stock NavigationView presenter (`ContentGrid`/`ContentPresenter` margins), not page spacing tokens.
